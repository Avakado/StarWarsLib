...
import { Planet } from '../models/swapi.interfaces';
...
export const PlanetsStore = signalStore(
  ...
  withMethods((store) => {
    ...
    return {
      loadPlanets: rxMethod<{ page: number; search: string }>(
        pipe(
          tap(({ page }) => patchState(store, { loading: true, currentPage: page, error: null })),
          switchMap(({ page, search }) =>
            swapiService.getPlanets(page, search).pipe(
              tap((res) =>
                patchState(store, {
                  planets: res.results,
                  hasNext: !!res.next,
              ...
              catchError(() => {
                patchState(store, { loading: false, error: 'Failed to load planets' });
                return EMPTY;
              }),
            ),
          ),
